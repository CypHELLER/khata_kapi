...
class SalesItem extends StatefulWidget {
  ...
  @override
  State<SalesItem> createState() => SalesItemState();
}

class SalesItemState extends State<SalesItem> {
  String uID = "";
  ...
  Future createBill() async {
    try {
      final docUser = FirebaseFirestore.instance.collection("sales").doc(uID);
      ...
      if (e.code == "not-found") {
        await FirebaseFirestore.instance.collection("sales").doc(uID).set({
          "sales": FieldValue.arrayUnion([
            {
              "quantity": int.parse(_quantityController.text),
              "rate": int.parse(_rateController.text),
